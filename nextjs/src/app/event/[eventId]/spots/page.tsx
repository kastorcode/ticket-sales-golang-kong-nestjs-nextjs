...
import { TicketKindSelect, TicketKindSelectProps } from "./TicketKindSelect"

async function getSpots (eventId : string) : Promise<{event : EventModel, spots : SpotModel[]}> {
...
export default async function SpotsPage ({ params } : SpotsPageProps) {
  ...
  const uniqueRows = [...new Set(rowLetters)]
  const spotGroupedByRow = uniqueRows.map(row => ({
    row,
    spots: [...spots.filter(spot => spot.name[0] === row).sort((a, b) => {
      const aInt = parseInt(a.name.slice(1))
      const bInt = parseInt(b.name.slice(1))
    ...
    <main className="mt-10">
      ...
      <div className="mt-6 flex flex-wrap justify-between">
        <div className=" mb-4 flex w-full max-w-[650px] flex-col gap-y-8 rounded-2xl bg-secondary p-6">
          <div className="rounded-2xl bg-bar py-4 text-center text-[20px] font-bold uppercase text-default">
            Stage
          </div>
          ...
          <div className="flex w-full flex-row justify-around">
            <div className=" flex flex-row items-center">
              <span className="mr-1 block h-4 w-4 rounded-full bg-available" />
              Available
            </div>
            <div className="flex flex-row items-center">
              <span className="mr-1 block h-4 w-4 rounded-full bg-occupied" />
              Occupied
            </div>
            <div className="flex flex-row items-center">
              <span className="mr-1 block h-4 w-4  rounded-full bg-chosen" />
              Chosen
            </div>
          </div>
        </div>
        <div className="flex w-full max-w-[478px] flex-col gap-y-6 rounded-2xl bg-secondary p-6">
          ...
          <div className="flex flex-col">
            {<TicketKindSelect
              defaultValue={ticketKind as TicketKindSelectProps["defaultValue"]}
              price={event.price}
            />}
          </div>
          ...
          <Link
            href="/checkout"
            className="rounded-lg bg-btn-primary py-4 text-sm font-semibold uppercase text-btn-primary text-center hover:text-btn-primary-hover hover:bg-btn-primary-hover"
          >
            Go to payment
          </Link>
        </div>
